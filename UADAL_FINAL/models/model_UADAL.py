...
Source_train = pd.read_csv("/content/drive/MyDrive/OSDA_Papers_adaptation/UADAL_FINAL/data/Source_train.csv")
Source_test = pd.read_csv("/content/drive/MyDrive/OSDA_Papers_adaptation/UADAL_FINAL/data/Source_test.csv")
Target_train = pd.read_csv("/content/drive/MyDrive/OSDA_Papers_adaptation/UADAL_FINAL/data/Target_train.csv")
Target_test = pd.read_csv("/content/drive/MyDrive/OSDA_Papers_adaptation/UADAL_FINAL/data/Target_test.csv")
...
train_source_loader = torch.utils.data.DataLoader(Source_train_dset, batch_size=32, num_workers=1)
train_target_loader = torch.utils.data.DataLoader(Target_train_dset, batch_size=32, num_workers=1)
...
class UADAL():
    def __init__(self, args, num_class, src_dset, target_dset):
        ...
        self.known_num_class = num_class
    ...
    def train_init(self):
        ...
        while step < self.args.warmup_iter + 1:
            ...
            self.C.train()
            epoch_cnt +=1
            for i, (img_s_aug,label_s) in enumerate(train_source_loader):
                #print(label_s.shape)
                if self.args.cuda:
                    img_s =img_s_aug
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.training_iter):
            ...
            for i, (img_t, label_t) in enumerate(train_target_loader):
                img_s = img_t
                label_s = label_t
                img_t_og = img_t
                img_t_aug = img_t
    ...
    def test(self, epoch):
        ...
        acc_unknown = sum([cm[i][self.known_num_class] for i in range(self.known_num_class, int(np.max(total_label_t)+1))]) / 1#(max_target_label - self.known_num_class)
