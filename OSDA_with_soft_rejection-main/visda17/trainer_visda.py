...
#import data_loader.get_loader as get_loader
import numpy as np
import os
...
parser.add_argument('--net', type=str, default='res', metavar='B',
...
source_path = "/content/drive/MyDrive/OSDA_Papers_adaptation/OSDA_with_soft_rejection-main/visda17/data/Source_train.csv"
target_path = "/content/drive/MyDrive/OSDA_Papers_adaptation/OSDA_with_soft_rejection-main/visda17/data/Target_train.csv"
evaluation_path = "/content/drive/MyDrive/OSDA_Papers_adaptation/OSDA_with_soft_rejection-main/visda17/data/Target_test.csv"
...
train_loader, test_loader, unique_labels, len_features = get_loader(source_path, target_path, evaluation_path,
...
model_f, model_c1, model_c2 = get_model(args.net,len_features = len_features, num_class=num_class, unit_size=args.unit_size)
model_unk = disc().cuda()
...
###### STEP 1 ######
        ...
        if with_detach:
            if softmax_input_unk:
                #print(torch.zeros_like(ind).view(-1,1).shape)
                unk_loss += criterion_bce(model_unk(F.softmax(output_s1, dim=1).index_select(0, ind).detach()), torch.zeros_like(ind).view(-1,1).float().cuda())
                unk_loss += criterion_bce(model_unk(F.softmax(output_s2, dim=1).index_select(0, ind).detach()), torch.zeros_like(ind).view(-1,1).float().cuda())
            else:
                unk_loss += criterion_bce(model_unk(output_s1.index_select(0, ind).detach()), torch.zeros_like(ind).view(-1,1).float().cuda())
                unk_loss += criterion_bce(model_unk(output_s2.index_select(0, ind).detach()), torch.zeros_like(ind).view(-1,1).float().cuda())
        else:
            if softmax_input_unk:
        ...
        if len(ind_list)!=0:
            ...
            if with_detach:
                if softmax_input_unk:
                    unk_loss += criterion_bce(model_unk(F.softmax(output_s1, dim=1).index_select(0, ind).detach()), torch.ones_like(ind).view(-1,1).float().cuda())
                    unk_loss += criterion_bce(model_unk(F.softmax(output_s2, dim=1).index_select(0, ind).detach()), torch.ones_like(ind).view(-1,1).float().cuda())
                else:
                    unk_loss += criterion_bce(model_unk(output_s1.index_select(0, ind).detach()), torch.ones_like(ind.view(-1,1)).float().cuda())
                    unk_loss += criterion_bce(model_unk(output_s2.index_select(0, ind).detach()), torch.ones_like(ind).view(-1,1).float().cuda())
            else:
                if softmax_input_unk:
